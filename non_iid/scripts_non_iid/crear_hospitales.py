...
usando una partición Non-IID severa (Distribución de Dirichlet).
"""
...
CONFIG_PATH = os.path.join(os.path.dirname(__file__), "..", "config_non_iid.yaml")
ALPHA_DIRICHLET = 0.3  # Heterogeneidad fuerte
...
def plan_dirichlet(N, n_h, alpha):
    proportions = np.random.dirichlet(np.repeat(alpha, n_h))
    cuotas = proportions * N
    pisos = np.floor(cuotas).astype(int)
    resto = N - pisos.sum()
    if resto > 0:
        indices_extra = np.random.choice(n_h, resto, replace=False)
...
def main():
    ...
    print(f"== Reparto NON-IID (Dirichlet alpha={ALPHA_DIRICHLET}) ==")
    ...
    for cls in clases:
        ...
        plan = plan_dirichlet(N, n_h, ALPHA_DIRICHLET)
