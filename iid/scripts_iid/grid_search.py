...
def run_one_config(method, hyperparams, hospitals, hospital_loaders,
    ...
    for round_idx in range(MAX_ROUNDS):
        ...
        if prev_val_acc is None:
            ...
            delta_prev_str = f"{avg_val_acc - prev_val_acc:+.4f}"
        delta_best = avg_val_acc - best_val_acc
        print(
            f"    Ronda {round_idx + 1:02d}/{MAX_ROUNDS} | "
            f"val_acc={avg_val_acc:.4f} | "
            f"delta_prev={delta_prev_str} | "
            f"delta_best={delta_best:+.4f} | "
            f"sin_mejora={rounds_without_improvement}/{PATIENCE}",
            flush=True
        )
...
def main():
    ...
    EPOCHS_VALUES = [1, 3, 5, 10]

    grids = {
        "fedavg": [
            {"lr": lr, "local_epochs": ep}
            for lr, ep in itertools.product(LR_VALUES, EPOCHS_VALUES)
        ],
        "fedprox": [
            {"lr": lr, "local_epochs": ep, "mu": mu}
            for lr, ep, mu in itertools.product(
                LR_VALUES, EPOCHS_VALUES, [0.001, 0.01, 0.1, 1.0]
            )
        ],
        "fedewa": [
            {"lr": lr, "local_epochs": ep}
        ...
        "fedag": [
            {"lr": lr, "local_epochs": ep, "lr_beta": lb}
            for lr, ep, lb in itertools.product(
                LR_VALUES, EPOCHS_VALUES, [0.001, 0.01, 0.1]
            )
        ],
        "mh_cro": [
            {"lr": lr, "local_epochs": ep, "lambda_reg": lam, "gamma_reg": gam}
            for lr, ep, lam, gam in itertools.product(
                LR_VALUES, EPOCHS_VALUES,
